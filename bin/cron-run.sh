...
# bus-only roster (12 jobs) — comfortably under the 20-check free tier, so
# everything except the low-stakes weekly/monthly recap is watched.
HC_MONITORED="observe-buses bus-cross-bunching bus-bunching bus-gaps bus-thin-gaps bus-ghosts bus-speedmap bus-fleet-rollup bus-alerts bus-fare-waiver fetch-gtfs audit-alerts"
