...
const COTA_URL = 'https://www.cota.com/';

// Alert-style posts may mention the bare domain "cota.com" in their body,
// but image/video/text posts carry no facets, so the domain isn't tappable.
// Build a richtext link facet over that mention pointing at the COTA homepage.
...
function cotaLinkFacets(text) {
  const needle = 'cota.com';
  ...
  return [
    {
      ...
      features: [{ $type: 'app.bsky.richtext.facet#link', uri: COTA_URL }],
    },
  ];
}
...
async function postWithImage(agent, text, imageBuffer, altText, replyRef = null) {
  ...
  const facets = cotaLinkFacets(text);
